...
interface VerifiedDisasterNewsRow {
  id: string;
  title: string;
  summary: string;
  steps: string[];
...
const VERIFIED_AT = '2026-08-15T12:00:00-05:00';

const OBSOLETE_SOURCE_URLS = [
...
const VERIFIED_DISASTER_NEWS: VerifiedDisasterNewsRow[] = [
  {
...
export class VerifiedDisasterNews1786960000000 implements MigrationInterface {
  name = 'VerifiedDisasterNews1786960000000';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `UPDATE "public_news" SET "status" = 'archived', "updatedAt" = now() WHERE "sourceUrl" = ANY($1::text[])`,
      [OBSOLETE_SOURCE_URLS],
    );

    for (const news of VERIFIED_DISASTER_NEWS) {
      await queryRunner.query(
        `INSERT INTO "public_news" ("id", "title", "summary", "steps", "requirements", "category", "department", "municipality", "sourceName", "sourceUrl", "contactInfo", "publishedAt", "validUntil", "verifiedAt", "featured", "status")
         SELECT $1::uuid, $2, $3, $4::jsonb, $5::jsonb, $6::"public"."public_news_category_enum", $7, $8, $9, $10::varchar, $11, $12::timestamptz, $13::timestamptz, $14::timestamptz, $15, 'published'
         WHERE NOT EXISTS (SELECT 1 FROM "public_news" WHERE "sourceUrl" = $10::varchar)
         ON CONFLICT ("id") DO NOTHING`,
        [
          news.id,
          news.title,
          news.summary,
          JSON.stringify(news.steps),
          JSON.stringify(news.requirements),
          news.category,
          news.department,
          news.municipality,
          news.sourceName,
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DELETE FROM "public_news" WHERE "id" = ANY($1::uuid[])`,
      [VERIFIED_DISASTER_NEWS.map(({ id }) => id)],
    );
    // Las publicaciones generales archivadas no se reactivan: volver a exponerlas
    // como noticias de desastre sería una regresión de contenido.
  }
}
